...
<main class="content">
	<section class="col-lg-9 col-md-9 col-sm-12 left-side textpage">
		...
		<nav itemscope itemtype="https://schema.org/WebPage">
		  <ul class="pagination">
		    <li>
		      <a href="<?=$c["website.base"]?>ლოგები/?pn=<?=$prev?>" aria-label="Previous">
		    ...
		    for($x=1; $x<=$maxpage; $x++){
		    	...
		    	echo '<li'.$active.'><a href="'.$c["website.base"].'ლოგები/?pn='.$x.'" itemprop="relatedLink/pagination">'.$x.'</a></li>'; 	
		    }
		    ?>
		    
		    <li>
		      <a href="<?=$c["website.base"]?>ლოგები/?pn=<?=$next?>" aria-label="Next">
